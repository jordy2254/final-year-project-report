...
For the rest API, it has been that Golang will be used. This is because it's a good use of goes modular approach, and because the project is quite light using something like \& spring is overkill for the scenario. Golang also has comparable performance to Java and is expandable, and is slowly becoming a more used languages due to it's speed and simplicity.

\subsubsection{Web Application}
The web Application will use React to develop it. This is because the author has an interest in keeping up with current popular technologies and feels this would be a good chance to implement the library and develop a progressive web app.
...
Using a database to store data is great for quick access and high yield options however creating a database to store a handful of maps on the android device will require a more complicated infrastructure with no real benefit as storage won't be accessed or read in the same way because the android application only needs to read map data, and write/update map data from the API, where a synchronisation will provide the JSON for the whole map overwriting any existing data. For this reason maps received from the rest API will be serialised into JSON files on the device. 
